...
interface PlayerData {
    ...
    dailyShareDate?: string;
    adDate?: string;
    adCount?: number;
    shareDate?: string;
    shareCount?: number;
    lastDailySharePopupDate?: string;
    settings: { bgm: boolean; sfx: boolean; vibration: boolean; nickname?: string; dailyShareDate?: string; };
...
try {
    ...
    if (saved) {
        ...
        if (!playerData.dailyShareDate) playerData.dailyShareDate = '';
    ...
    wx.cloud.callContainer({
        ...
        success: (res: any) => {
            if (res.statusCode === 200 && res.data) {
                ...
                const cloudTime = cloudData.lastUpdated || (cloudData.updated_at ? new Date(cloudData.updated_at).getTime() : 0);
                const localTime = playerData.lastUpdated || 0;
                // Add a small buffer (500ms) to avoid flip-flopping if the timestamps are extremely close due to network delays
                const cloudIsNewer = cloudTime > (localTime + 500);
                ...
                if (cloudIsNewer || cloudData.level > playerData.level || (playerData.level === 1 && cloudData.level > 0)) {
                    ...
                    if (cloudData.settings && cloudData.settings.trim() !== '') {
                        try {
                            playerData.settings = { ...playerData.settings, ...JSON.parse(cloudData.settings) };
                            if (playerData.settings.dailyShareDate) {
                                playerData.dailyShareDate = playerData.settings.dailyShareDate;
                            }
                            if (playerData.settings.nickname) {
                                wx.setStorageSync('playerNickname', playerData.settings.nickname);
                ...
                // 每次登录初始化完毕后，自动向排行榜上报一次最新的名字和等级
                let currentName = '';
                try { currentName = wx.getStorageSync('playerNickname') || playerData.settings.nickname || '匿名玩家'; } catch (e) { }
                if (currentName && !currentName.startsWith('玩家_')) {
    ...
    if (typeof wx !== 'undefined' && wx.cloud) {
        ...
        wx.cloud.callContainer({
            ...
            data: {
                ...
                settings: JSON.stringify({ ...playerData.settings, dailyShareDate: playerData.dailyShareDate }),
...
const submitToGlobalLeaderboard = (name: string, mode: string = 'main', score: number = 0) => {
    console.log(`[排行榜同步] 正在上传数据 -> 昵称: ${name}, 分数: ${score}`);
    wx.cloud.callContainer({
        config: { env: 'prod-d5gnecgcl8574e82a' },
...
const SHOP_ITEMS = {
    ...
    props: [
        { id: 'undo', name: '撤回道具', price: 100, isConsumable: true, packSize: 3 },
        { id: 'extract', name: '移出道具', price: 150, isConsumable: true, packSize: 3 },
        { id: 'shuffle', name: '洗牌道具', price: 120, isConsumable: true, packSize: 3 },
        { id: 'bundle', name: '超值大礼包', price: 300, isConsumable: true, desc: '包含：各x3' }
    ],
    coins: [
...
const dailyShareContainer = new PIXI.Container();
...
app.stage.addChild(dailyShareContainer);
...
dailyShareContainer.visible = false;
...
// ================= 0.7. 每日首登分享 (Daily Share) =================
let pendingDailyShareReward = false;
const shareMessages = [
    "😱 只有 1% 的人能通关的智力挑战，你能挺过第 20 关吗？",
    "🔥 停不下来！这个果宝消除游戏太上头了，快来比比谁闯的关数多！",
    "🆘 我在这里卡了一整天，求求各路大神帮帮忙！"
];

const dailyShareOverlay = new PIXI.Graphics();
dailyShareOverlay.beginFill(0x000000, 0.8);
dailyShareOverlay.drawRect(0, 0, screenWidth, screenHeight);
dailyShareOverlay.endFill();
dailyShareOverlay.interactive = true;
dailyShareOverlay.on('pointerdown', () => { dailyShareContainer.visible = false; });
dailyShareContainer.addChild(dailyShareOverlay);

const dsPanel = new PIXI.Container();
dsPanel.position.set(screenWidth / 2, screenHeight / 2);
dsPanel.interactive = true;
dailyShareContainer.addChild(dsPanel);

// 动态背景板 (匹配游戏的暖色调)
const dsBg = new PIXI.Graphics();
dsBg.beginFill(0xFFFAF0);
dsBg.lineStyle(4, 0xF59E0B, 1);
dsBg.drawRoundedRect(-160, -220, 320, 440, 20);
dsBg.endFill();
dsPanel.addChild(dsBg);

// 核心标题 (2倍渲染，缩放0.5解决模糊)
const dsTitleText = new PIXI.Text('🎁 每日免费豪礼', { fontFamily: '"PingFang SC"', fontSize: 52, fill: '#D97706', fontWeight: 'bold' });
dsTitleText.anchor.set(0.5);
dsTitleText.position.set(0, -170);
dsTitleText.scale.set(0.5);
dsPanel.addChild(dsTitleText);

// 呼吸动画
let dsTime = 0;
app.ticker.add(() => {
    if (dailyShareContainer.visible) {
        dsTime += 0.05;
        dsTitleText.scale.set(0.5 + Math.sin(dsTime) * 0.025);
        dsCards.forEach((dc, i) => {
            dc.glow.alpha = 0.7 + Math.sin(dsTime * 2 + i) * 0.3;
        });
    }
});

// 诱惑力文案 (超清渲染)
const dsDesc = new PIXI.Text('动动手指，发到群里炫耀一下\n即刻到账价值 900 金币的通关神器！', {
    fontFamily: '"PingFang SC"', fontSize: 64, fill: '#78350F', align: 'center', lineHeight: 104, fontWeight: 'bold'
});
dsDesc.anchor.set(0.5);
dsDesc.scale.set(0.25);
dsDesc.y = -100;
dsPanel.addChild(dsDesc);

// 高光奖励展示
const dsRewardContainer = new PIXI.Container();
dsRewardContainer.y = -30; // Move up even more
const dsIcons = ['🔙', '📤', '🔀'];
const dsNames = ['撤回道具', '移出道具', '洗牌道具'];
const dsCards: { card: PIXI.Container, glow: PIXI.Graphics }[] = [];

for (let i = 0; i < 3; i++) {
    const card = new PIXI.Container();
    const cardW = 88;
    const cardH = 110;
    card.position.set(-138 + i * 94, -20); // Better centered, slightly wider gap

    // 1. Base & Border
    const base = new PIXI.Graphics();
    base.beginFill(0x000000, 0.08);
    base.drawRoundedRect(0, 4, cardW, cardH, 16);
    base.endFill();
    base.beginFill(0xFFFFFF);
    base.drawRoundedRect(0, 0, cardW, cardH, 16);
    base.endFill();
    card.addChild(base);
    ...
    mask.drawRoundedRect(3, 3, cardW - 6, cardH - 6, 13);
    mask.endFill();
    content.mask = mask;
    card.addChild(mask);
    ...
    // 3. BG Gradient
    const bgTex = getGradientTexture('#FFF8E1', '#FFECB3', cardW, cardH);
    ...
    innerBorder.drawRoundedRect(3, 3, cardW - 6, cardH - 6, 13);
    content.addChild(innerBorder);

    // 5. Glow Pedestal
    const iconContainer = new PIXI.Container();
    iconContainer.position.set(cardW / 2, cardH / 2 + 6);
    const glow = new PIXI.Graphics();
    const baseColor = 0xFFCA28;
    ...
    // 6. Emoji
    const previewText = new PIXI.Text(dsIcons[i], { fontSize: 72 });
    previewText.anchor.set(0.5);
    previewText.scale.set(0.5);
    previewText.style.dropShadow = true;
    previewText.style.dropShadowDistance = 2;
    previewText.style.dropShadowBlur = 2;
    ...
    badge.beginFill(0x8D6E63);
    badge.lineStyle(2, 0xFFFFFF, 1);
    badge.drawRoundedRect(cardW / 2 - 40, -10, 80, 20, 10);
    badge.endFill();
    card.addChild(badge);

    const badgeText = new PIXI.Text(dsNames[i], { fontFamily: '"PingFang SC"', fontSize: 20, fill: '#FFFFFF', fontWeight: 'bold' });
    ...
    actionBg.beginFill(0xFFCA28);
    actionBg.lineStyle(2, 0xFFFFFF, 0.8);
    actionBg.drawRoundedRect(cardW / 2 - 25, cardH - 26, 50, 22, 11);
    actionBg.endFill();
    card.addChild(actionBg);

    const actionText = new PIXI.Text('+3', { fontSize: 24, fill: '#FFFFFF', fontWeight: '900' });
    ...
    actionText.scale.set(0.5);
    actionText.position.set(cardW / 2, cardH - 15);
    card.addChild(actionText);

    dsCards.push({ card, glow });
    dsRewardContainer.addChild(card);
}
dsPanel.addChild(dsRewardContainer);

// 按钮设计 (动态按钮)
const dsShareBtn = new PIXI.Container();
dsShareBtn.position.set(0, 130);
const dsShareBg = new PIXI.Graphics();
dsShareBg.beginFill(0x10B981);
dsShareBg.drawRoundedRect(-90, -30, 180, 60, 30);
dsShareBg.endFill();
const dsShareTxt = new PIXI.Text('⚡ 立即炫耀领奖', { fontFamily: '"PingFang SC"', fontSize: 40, fill: '#FFFFFF', fontWeight: 'bold' });
dsShareTxt.anchor.set(0.5);
dsShareTxt.scale.set(0.5);
dsShareBtn.addChild(dsShareBg, dsShareTxt);
dsShareBtn.interactive = true;
dsShareBtn.buttonMode = true;

app.ticker.add(() => {
    if (dailyShareContainer.visible) {
        dsShareBtn.scale.set(1 + Math.sin(dsTime * 1.5) * 0.05);
    }
});
dsPanel.addChild(dsShareBtn);

// 关闭按钮
const dsCloseBtn = new PIXI.Container();
dsCloseBtn.position.set(160, -220);
const dsCloseBg = new PIXI.Graphics();
dsCloseBg.beginFill(0xEF4444);
dsCloseBg.drawCircle(0, 0, 16);
dsCloseBg.endFill();
const dsCloseX = new PIXI.Text('×', { fontFamily: 'Arial', fontSize: 20, fill: '#FFFFFF', fontWeight: 'bold' });
dsCloseX.anchor.set(0.5);
dsCloseBtn.addChild(dsCloseBg, dsCloseX);
dsCloseBtn.interactive = true;
dsCloseBtn.buttonMode = true;
dsCloseBtn.on('pointerdown', () => { dailyShareContainer.visible = false; });
dsPanel.addChild(dsCloseBtn);

dsShareBtn.on('pointerdown', () => {
    pendingDailyShareReward = true;
    const msg = shareMessages[Math.floor(Math.random() * shareMessages.length)];
    if (typeof wx !== 'undefined' && wx.shareAppMessage) {
        wx.shareAppMessage({ title: msg });
    } else {
        // Fallback for non-wx environment to test logic
        if (typeof wx !== 'undefined' && wx.showToast) {
            wx.showToast({ title: '环境不支持分享，模拟成功', icon: 'none' });
        }
        triggerDailyShareReward();
    }
});

const triggerDailyShareReward = () => {
    const today = getOffsetDateInfo(0).key.replace('daily_score_', '');
    if (playerData.dailyShareDate !== today) {
        playerData.props.undo += 3;
        playerData.props.extract += 3;
        playerData.props.shuffle += 3;
        playerData.dailyShareDate = today;
        savePlayerData();
        ...
        if (typeof wx !== 'undefined' && wx.showToast) {
            wx.showToast({ title: '分享成功，道具各+3！', icon: 'none', duration: 2000 });
        }
        dailyShareContainer.visible = false;
    }
    pendingDailyShareReward = false;
};

if (typeof wx !== 'undefined' && wx.onShow) {
    wx.onShow(() => {
        if (pendingDailyShareReward) {
            setTimeout(() => {
                if (pendingDailyShareReward) triggerDailyShareReward();
            }, 500);
        }
    });
}
...
function initHomeScreen() {
    ...
    // 3. 免费福利按钮 (Daily Share manual trigger)
    const dailyGiftBtn = new PIXI.Container();
    dailyGiftBtn.x = screenWidth - 45; // 右侧对齐
    dailyGiftBtn.y = screenHeight * 0.88; // 右下角草地上

    const giftBg = new PIXI.Graphics();
    giftBg.beginFill(0xFF9800); // 活力橙
    giftBg.lineStyle(2, 0xFFFFFF, 1);
    giftBg.drawCircle(0, 0, 22);
    giftBg.endFill();

    const giftIcon = new PIXI.Text('🎁', { fontSize: 22 });
    giftIcon.anchor.set(0.5);
    giftIcon.position.set(0, -6);

    const giftLabelBg = new PIXI.Graphics();
    giftLabelBg.beginFill(0xEF4444);
    giftLabelBg.drawRoundedRect(-20, 7, 40, 16, 8);
    giftLabelBg.endFill();

    const giftLabel = new PIXI.Text('免费福利', {
        fontFamily: '"PingFang SC"',
        fontSize: 9,
        fill: '#FFFFFF',
        fontWeight: 'bold'
    ...
    giftLabel.position.set(0, 15);

    dailyGiftBtn.addChild(giftBg, giftIcon, giftLabelBg, giftLabel);
    dailyGiftBtn.interactive = true;
    dailyGiftBtn.buttonMode = true;
    dailyGiftBtn.on('pointerdown', () => { dailyGiftBtn.alpha = 0.7; });
    dailyGiftBtn.on('pointerup', () => {
        dailyGiftBtn.alpha = 1;
        dailyShareContainer.visible = true;
    });
    dailyGiftBtn.on('pointerupoutside', () => { dailyGiftBtn.alpha = 1; });

    // Animate the button to draw attention if reward not claimed
    let giftTime = 0;
    app.ticker.add(() => {
        if (!homeContainer.visible) return;
        const today = getOffsetDateInfo(0).key.replace('daily_score_', '');
        if (playerData.dailyShareDate !== today) {
            dailyGiftBtn.visible = true;
            giftTime += 0.05;
            dailyGiftBtn.scale.set(1 + Math.sin(giftTime) * 0.05);
            dailyGiftBtn.rotation = Math.sin(giftTime * 2.5) * 0.05;
        } else {
            dailyGiftBtn.visible = false;
        }
    });

    homeContainer.addChild(dailyGiftBtn);
    ...
    function renderSettingsScreen() {
        ...
        // 获取当前昵称和头像
        let nickname = '';
        try { nickname = wx.getStorageSync('playerNickname') || playerData.settings.nickname || '匿名玩家'; } catch (e) { nickname = playerData.settings.nickname || '匿名玩家'; }
            ...
            wx.showModal({
                ...
                success: (res) => {
                    if (res.confirm && res.content) {
                        const newName = res.content.substring(0, 12);
                        wx.setStorageSync('playerNickname', newName);
                        playerData.settings.nickname = newName;
                        savePlayerData();
        ...
        const versionText = new PIXI.Text('当前版本是v1.1.9', {
...
function createShopCard(item: any, category: string, isUnlocked: boolean, isEquipped: boolean, cardW: number, cardH: number): PIXI.Container {
    ...
    if (category === 'bgs') {
        ...
        if (category === 'tiles') {
            ...
            // Add packSize (+3) pill for consumables, matching Daily Share style
            if (item.packSize) {
                const actionBg = new PIXI.Graphics();
                actionBg.beginFill(0xFFCA28);
                actionBg.lineStyle(2, 0xFFFFFF, 0.8);
                actionBg.drawRoundedRect(-25, 20, 50, 22, 11); // Positioned at the bottom of the icon
                actionBg.endFill();
                iconContainer.addChild(actionBg);

                const actionText = new PIXI.Text(`+${item.packSize}`, { fontSize: 24, fill: '#FFFFFF', fontWeight: '900' });
                actionText.style.dropShadow = true;
                actionText.style.dropShadowDistance = 1;
                actionText.style.dropShadowAlpha = 0.2;
                actionText.anchor.set(0.5);
                actionText.scale.set(0.5);
                actionText.position.set(0, 31);
                iconContainer.addChild(actionText);
            }
    ...
    if (item.isConsumable && item.id !== 'bundle' && (playerData.props as any)[item.id] !== undefined) {
        ...
        countBg.drawRoundedRect(-22, -10, 44, 20, 10);
        countBg.endFill();
        ...
        // Move to top right corner as requested
        countContainer.position.set(cardW - 24, 16);
    ...
    // 5. Top Badge (Name)
    const badgeText = new PIXI.Text(item.name, { fontSize: 13, fill: '#FFFFFF', fontWeight: 'bold' });
    badgeText.anchor.set(0.5);
    badgeText.position.set(cardW / 2, 1);

    // Dynamically calculate width to prevent text overflow, with a smaller minimum to avoid overlap with top-right badges
    const badgeW = Math.max(72, badgeText.width + 24);

    const badge = new PIXI.Graphics();
    badge.beginFill(0x8D6E63); // Warm premium brown
    badge.lineStyle(2, 0xFFFFFF, 1); // Clean white border
    badge.drawRoundedRect(cardW / 2 - badgeW / 2, -10, badgeW, 22, 11);
    badge.endFill();
    card.addChild(badge);
    card.addChild(badgeText);
...
function checkAllReady() {
    ...
    if (assetsLoaded && !isUserDataLoaded) {
        ...
        setTimeout(() => {
            ...
            const today = getOffsetDateInfo(0).key.replace('daily_score_', '');
            if (playerData.dailyShareDate !== today && playerData.lastDailySharePopupDate !== today) {
                playerData.lastDailySharePopupDate = today;
                savePlayerData();

                // 加一个 500ms 的延迟，让玩家先看清主界面，弹窗显得没那么突兀
                setTimeout(() => {
                    dailyShareContainer.visible = true;
                }, 500);
            }
